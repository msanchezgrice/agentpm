...
INSERT INTO trades (
    ...
    (gen_random_uuid(), '00000000-0000-0000-0000-000000000001', 'NVDA', 'buy', 150, 425.50, NOW() - INTERVAL '15 days',
    ...
    (gen_random_uuid(), '00000000-0000-0000-0000-000000000001', 'TSLA', 'buy', 200, 178.30, NOW() - INTERVAL '8 days',
    ...
    (gen_random_uuid(), '00000000-0000-0000-0000-000000000001', 'META', 'buy', 100, 385.20, NOW() - INTERVAL '22 days',
    ...
    (gen_random_uuid(), '00000000-0000-0000-0000-000000000001', 'SNAP', 'buy', 500, 15.80, NOW() - INTERVAL '30 days',
    ...
    (gen_random_uuid(), '00000000-0000-0000-0000-000000000001', 'SNAP', 'sell', 500, 13.30, NOW() - INTERVAL '25 days',
    ...
    -- More winning trades
    (gen_random_uuid(), '00000000-0000-0000-0000-000000000001', 'GOOGL', 'buy', 80, 142.50, NOW() - INTERVAL '5 days',
    ...
    (gen_random_uuid(), '00000000-0000-0000-0000-000000000001', 'AAPL', 'buy', 100, 189.50, NOW() - INTERVAL '45 days',
    ...
    (gen_random_uuid(), '00000000-0000-0000-0000-000000000001', 'AAPL', 'sell', 100, 198.00, NOW() - INTERVAL '35 days',
...
UPDATE agents 
SET 
    total_trades = (SELECT COUNT(*) FROM trades WHERE agent_id = '00000000-0000-0000-0000-000000000001'),
    winning_trades = (SELECT COUNT(*) FROM trades WHERE agent_id = '00000000-0000-0000-0000-000000000001' AND trade_outcome > 0),
    losing_trades = (SELECT COUNT(*) FROM trades WHERE agent_id = '00000000-0000-0000-0000-000000000001' AND trade_outcome <= 0),
    win_rate = (
        SELECT COUNT(*) FILTER (WHERE trade_outcome > 0) * 100.0 / NULLIF(COUNT(*), 0)
        FROM trades WHERE agent_id = '00000000-0000-0000-0000-000000000001'
    )
...
-- Set up for other demo agents (for landing page comparison) - basic columns only
