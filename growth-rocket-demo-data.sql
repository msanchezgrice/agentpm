...
VALUES ('demo-user-001', 'demo@agentpm.ai', NOW(), NOW())
...
SELECT 
    ...
    daily_return,
    total_return_pct,
    FLOOR(RANDOM() * 3), -- 0-2 trades per day
...
SET 
    ...
    win_rate = (
        SELECT COUNT(*) FILTER (WHERE trade_outcome > 0) * 100.0 / COUNT(*) 
        FROM trades WHERE agent_id = 'growth-rocket-001'
    )
WHERE id = 'growth-rocket-001';

-- Add some sample agent reviews for social proof
...
INSERT INTO market_indices_performance (date, sp500_value, nasdaq_value, created_at) 
SELECT 
    date,
